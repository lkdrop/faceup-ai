...
export default function StepGender() {
  ...
  return (
    <div>
      <p className="text-xs font-semibold text-neutral-400 uppercase tracking-widest mb-3">Passo 1 de 4</p>
      <h2 className="text-2xl sm:text-3xl font-black text-[#111111] mb-2 tracking-tight">Quem vai nas fotos?</h2>
        ...
        {options.map(opt => {
          ...
          return (
            ...
            >
              ...
              <div className={cn('text-neutral-300 transition-colors mb-1', selected && 'text-[#FF7A1A]')}>
                {opt.icon}
              </div>
              <span className={cn('text-base font-bold leading-tight transition-colors', selected ? 'text-[#111111]' : 'text-neutral-700')}>
                {opt.label}
              </span>
              <span className="text-sm text-neutral-400 leading-snug">{opt.desc}</span>
            </button>
          )
        })}
      </div>
